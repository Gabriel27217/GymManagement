...
namespace GymManagement.Web.Pages.Salas
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task OnGetAsync()
        {
            ...
            {
                var h = Http();
                var j = await h.GetStringAsync("api/Salas");
                Salas = JsonSerializer.Deserialize<List<SalaDto>>(j, Opts()) ?? new();
            }
            catch { ModelState.AddModelError("", "Não foi possível carregar as salas."); }
        }
        private HttpClient Http() { var h = _f.CreateClient("GymAPI"); h.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("JWT")); return h; }
    ...
    public class CreateModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var h = Http();
            // FIX #3: envia apenas campos do DTO, sem Id
            var body = new { Nome, CapacidadeMaxima, Descricao, Ativa = true };
    ...
    public class EditModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            if (!ModelState.IsValid) return Page();
            var h = Http();
            // FIX #3: envia apenas campos do DTO, sem Id no body
            var body = new { Nome, CapacidadeMaxima, Descricao, Ativa };
            var resp = await h.PutAsync($"api/Salas/{SalaId}", new StringContent(JsonSerializer.Serialize(body), Encoding.UTF8, "application/json"));
            if (!resp.IsSuccessStatusCode) { ModelState.AddModelError("", "Erro ao atualizar sala."); return Page(); }
    ...
    public class DeleteModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(int id)
        {
            ...
            {
                var m = JsonSerializer.Deserialize<Dictionary<string,string>>(await resp.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                TempData["Erro"] = m?.GetValueOrDefault("mensagem") ?? "Erro ao eliminar.";
            }
...
namespace GymManagement.Web.Pages.Utilizadores
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task OnGetAsync()
        {
            ...
            {
                var h = Http();
                var j = await h.GetStringAsync("api/Utilizadores");
                Utilizadores = JsonSerializer.Deserialize<List<UtilizadorListDto>>(j, new JsonSerializerOptions { PropertyNameCaseInsensitive = true }) ?? new();
            }
            catch { ModelState.AddModelError("", "Não foi possível carregar os utilizadores."); }
        }
        private HttpClient Http() { var h = _f.CreateClient("GymAPI"); h.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("JWT")); return h; }
    }
    ...
    public class DeleteModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(int id)
        {
            var h = _f.CreateClient("GymAPI");
            h.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", HttpContext.Session.GetString("JWT"));
            var resp = await h.DeleteAsync($"api/Utilizadores/{id}");
            if (resp.IsSuccessStatusCode)
                TempData["Sucesso"] = "Utilizador eliminado.";
            else
            {
                var m = JsonSerializer.Deserialize<Dictionary<string,string>>(await resp.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                TempData["Erro"] = m?.GetValueOrDefault("mensagem") ?? "Não foi possível eliminar o utilizador.";
            }
            return RedirectToPage("Index");
        }
    }
...
namespace GymManagement.Web.Pages.Frequencias
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task OnGetAsync()
        {
            // FIX #8: usar User.IsInRole em vez de Session para verificar role
            ...
            catch { ModelState.AddModelError("", "Não foi possível carregar as frequências."); }
        }
